...
/**
 * Description of NestesSetsTreeBehavior
 *
 * @author kotov
 */
class NestedSetsTreeBehavior extends Behavior
{
    ...
    public function tree($showAll = false)
    {
        ...
        // Trees mapped
        $trees = array();
        /** @var ActiveQuery $collectionQuery */
        $collectionQuery = $this->owner->find()->orderBy([$this->leftAttribute => SORT_ASC]);
        if ($showAll != true) {
            $collectionQuery->andFilterWhere(['deleted' => false]);
        }
        $collection = $collectionQuery->asArray()->all();

<template>
    <div class="form-group align-right" :class="{ 'required': required }">
        <div class="flex__wrapper">
            ...
            <input :id="id" type="text" class="form-control" :class="{ 'is-invalid': hasErrorsForShow() }" v-model="val"
                @change="onChange($event)" placeholder="">
        </div>
        <div v-if="hasErrorsForShow()" class="help-block">{{ errors.required.message }}</div>
        <div v-if="!hasErrorsForShow()" class="field__desc">{{ descriptionLabel }}</div>
    </div>
</template>
<script>

<template>
    <div class="form-group">
        ...
        <select @change="change" :name="id" :id="id" class="form-control" v-model="selected">
            <option v-for="element in enums" :value="element.id">{{
                getName(element.name, element.name_eng) }}</option>
        </select>
...
export default {
    ...
    created() {
        ...
        console.log(this.params.default_value);
        let defValId;
        if (this.params.default_value) {
            defValId = this.getIdForValue(this.params.default_value);
        }
        if (defValId) {
            this.selected = defValId;
        }
    },
    methods: {
        getIdForValue(val) {
